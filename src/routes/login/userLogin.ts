...
loginRoute.post("/", async (req: Request, res: Response) => {
    ...
    if(email !== null || email !== undefined || !email) queue.email = email as string;
    else if(username !== null || username !== undefined || !username) queue.username = username as string;
    ...
    if(isUser == null) return res.status(400).send("Wrong email!");
        ...
        if(!(await comparePassword(isUser.password, password))) return res.status(400).send("Wrong password");
